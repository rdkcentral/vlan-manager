...
- **R-BUS**: R-BUS registration with namespace "eRT.com.cisco.spvtg.ccsp.vlanmanager"
...
The VLAN Manager implements a single-threaded event-driven architecture optimized for deterministic network configuration operations. The component operates within a single main thread that processes R-BUS events, TR-181 parameter operations, and HAL responses sequentially to avoid race conditions and ensure atomic VLAN configuration changes.

- **Threading Architecture**: Single-threaded with event loop processing
- **Main Thread**: Handles R-BUS events, TR-181 parameter get/set operations, HAL JSON-RPC communication, PSM persistence operations, and configuration file management
  - **Event Processing**: Uses R-BUS event loop for asynchronous handling of parameter requests, component lifecycle events, and inter-component notifications
...
sequenceDiagram
    ...
    participant MessageBus as R-BUS
...
- **Parameter Registration**: Parameters are registered with R-BUS during component initialization using XML data model definitions in `RdkVlanManager.xml`. Registration includes function pointers for get/set operations and parameter metadata.
- **Access Mechanism**: External components access parameters via R-BUS using standard TR-181 paths. Internal access uses direct DML function calls for performance optimization.
...
| **System Service Provider** | Manages component lifecycle, R-BUS integration, and inter-component communication. Handles service registration and discovery.                                      | `ssp_main.c`, `ssp_messagebus_interface.c`          |
...
- **Event Processing**: Hardware events from the HAL layer are processed asynchronously through JSON-RPC callbacks, mapped to internal state changes, and propagated to dependent components through R-BUS events.
  ...
  - Asynchronous event processing through R-BUS integration
...
- **Error Handling Strategy**: Comprehensive error detection and recovery mechanisms ensure network stability during configuration failures, hardware errors, or component communication issues.
  ...
  - Timeout handling and retry logic for PSM operations and R-BUS communication
